...
governance_schema='data_governance'
...
def create_tags():
    try:
        ...
        cur.execute(f"use schema {database}.{governance_schema}")
    ...
    # print("checking if any tags are applied on rndcontrolling.DP_RDPORTFOLIO360 views")     
    # result=cur.execute(""" select * from table(INFORMATION_SCHEMA.TAG_REFERENCES_ALL_COLUMNS('rndcontrolling.DP_RDPORTFOLIO360.VW_MEDDRA_ICD10_MAPPING','table')) union all
    #                 select * from table(INFORMATION_SCHEMA.TAG_REFERENCES_ALL_COLUMNS('rndcontrolling.DP_RDPORTFOLIO360.VW_MEDDRA_SNOMEDCT_MAPPING','table')) union all
    #                 select * from table(INFORMATION_SCHEMA.TAG_REFERENCES_ALL_COLUMNS('rndcontrolling.DP_RDPORTFOLIO360.VW_PROJECT_INDICATION_FLAT','table')) union all
    #                 select * from table(INFORMATION_SCHEMA.TAG_REFERENCES_ALL_COLUMNS('rndcontrolling.DP_RDPORTFOLIO360.VW_PROJECT_INDICATION_TPP','table')) union all
    #                 select * from table(INFORMATION_SCHEMA.TAG_REFERENCES_ALL_COLUMNS('rndcontrolling.DP_RDPORTFOLIO360.VW_PROJECT_TCP','table')) union all
    #                 select * from table(INFORMATION_SCHEMA.TAG_REFERENCES_ALL_COLUMNS('rndcontrolling.DP_RDPORTFOLIO360.VW_PROJECT_TEAM_MEMBER','table')) union all
    #                 select * from table(INFORMATION_SCHEMA.TAG_REFERENCES_ALL_COLUMNS('rndcontrolling.DP_RDPORTFOLIO360.VW_PROJECT_TVP','table'))""")
    # status=cur.fetchone()
    # if status:
    #     for row in result.fetchall():
    #         print(row)
    # else:
    #     print("No tags are applied currently")
def Apply_tags():
    # Read Excel file
    # -----------------------------
    file_path = file
    ...
    for _, row in df.iterrows():
        ...
        for column in column_list:
            ...
            SET TAG {database}.{governance_schema}.{TAG_NAME} = '{sensitivity_level}'
            """
